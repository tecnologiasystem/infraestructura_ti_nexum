...
def listar_encabezados():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("EXEC [dbo].[SP_CRUD_EmailImagenes] @Accion = 5")
...
def listar_detalles_por_encabezado(id_encabezado):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("EXEC [dbo].[SP_CRUD_EmailImagenes] @Accion = 6, @idEncabezado = ?", (id_encabezado,))
    rows = cursor.fetchall()
    columnas = [col[0] for col in cursor.description]
    cursor.close()
    conn.close()
...
def dashboard_resumen(
    ...
    LEFT JOIN dbo.EmailImagenesEncabezado e ON e.idEncabezado = d.idEncabezado
...
def listar_detalles_por_encabezado_con_click(id_encabezado: int):
    conn = get_connection()
    cursor = conn.cursor()
    ...
    SELECT
        ...
        d.fecha_envio,
        d.error_detalle,
        CASE WHEN cx.idDetalle IS NULL THEN 'NO' ELSE 'SI' END AS clickeo,
        cx.ip,
        cx.user_agent,
        cx.fecha_click
    FROM dbo.EmailImagenesDetalle d
    OUTER APPLY (
        SELECT TOP 1
            c.idDetalle, c.ip, c.user_agent, c.fecha_click
        FROM dbo.EmailImagenesClicks c
        WHERE c.idDetalle = d.idDetalle
        ORDER BY c.fecha_click DESC
    ) cx

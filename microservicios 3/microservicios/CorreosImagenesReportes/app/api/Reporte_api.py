...
from app.dal.Reporte_dal import (
    ...
    listar_detalles_por_encabezado_con_click,
    )
...
async def exportar_excel_por_encabezado(idEncabezado: int = Query(...)):
    try:
        data = listar_detalles_por_encabezado_con_click(idEncabezado)
        if not data:
        ...
        columnas_preferidas = [
            "email_destinatario", "asunto",
            "estado_envio","fecha_registro", "fecha_envio",
            "clickeo", "fecha_click", "error_detalle"
        ]

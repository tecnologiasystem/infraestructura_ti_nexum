...
@router.get("/whatsapp/pendientes-json-adamantine")
def get_pendientes_json_adamantine(
    estado: str = Query("ENVIADO", description="Estado a marcar al entregar al RPA"),
    limit: int = Query(1, ge=1, le=5000)
):
    try:
        return obtener_pendientes_y_marcar_por_campana("ADAMANTINE", limit=limit, estado=estado)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    
@router.get("/whatsapp/pendientes-json-credivalores")
def get_pendientes_json_credivalores(
    estado: str = Query("ENVIADO", description="Estado a marcar al entregar al RPA"),
    limit: int = Query(1, ge=1, le=5000)
):
    try:
        return obtener_pendientes_y_marcar_por_campana("CREDIVALORES", limit=limit, estado=estado)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

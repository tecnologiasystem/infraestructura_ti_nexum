...
from app.Runt.bll.runt_bll import (
    procesar_archivo_excel,
    procesar_resultado_automatizacionRunt,
    ResultadoRuntModel, enviar_correo_finalizacion_por_encabezado,
    pausar_encabezado, reanudar_encabezado
)
from app.Runt.dal.runt_dal import DetalleModel, EncabezadoModel, obtener_detalles_agrupados_Runt, obtener_detalles_por_encabezado
...
async def descargar_plantilla():
    """
    Función para descargar la plantilla Excel para cargar datos RUNT.
...
async def guardar_excel(
    ...
    """
    Esta función recibe el archivo Excel con los datos RUNT subidos por el usuario y procesa su almacenamiento e inserción.
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            detalles.append(DetalleModel(
                ...
                placaVehiculo=str(row.get("PLACA VEHICULO", "")).strip(),
                tipoServicio=str(row.get("TIPO DE SERVICIO", "")).strip(),
                estadoVehiculo=str(row.get("ESTADO DE VEHICULO", "")).strip(),
                claseVehiculo=str(row.get("CLASE DE VEHICULO", "")).strip(),
                marca=str(row.get("MARCA", "")).strip(),
                modelo=str(row.get("MODELO", "")).strip(),
                numeroSerie=str(row.get("NUMERO DE SERIE", "")).strip(),
                numeroChasis=str(row.get("NUMERO DE CHASIS", "")).strip(),
                cilindraje=str(row.get("CILINDRAJE", "")).strip(),
                tipoCombustible=str(row.get("TIPO DE COMBUSTIBLE", "")).strip(),
                autoridadTransito=str(row.get("AUTORIDAD DE TRANSITO", "")).strip(),
                linea=str(row.get("LINEA", "")).strip(),
                color=str(row.get("COLOR", "")).strip(),
                numeroMotor=str(row.get("NUMERO DE MOTOR", "")).strip(),
                numeroVIN=str(row.get("NUMERO DE VIN", "")).strip(),
                tipoCarroceria=str(row.get("TIPO DE CARROCERIA", "")).strip(),
                polizaSOAT=str(row.get("POLIZA SOAT", "")).strip(),
                revisionTecnomecanica=str(row.get("REVISION TECNOMECANICA", "")).strip(),
                limitacionesPropiedad=str(row.get("LIMITACIONES PROPIEDAD", "")).strip(),
                garantiasAFavorDe=str(row.get("GARANTIAS A FAVOR DE", "")).strip()
            ))
...
@router.post("/automatizacion/resultadoRunt", tags=["Automatizacion Runt"])
def recibir_resultado(resultado: ResultadoRuntModel):
    """
    Endpoint para recibir resultados de la automatización RUNT.
    ...
    try:
        updated = procesar_resultado_automatizacionRunt(resultado)
        if updated:
...
@router.get("/detalle/listar_agrupadoRunt", tags=["Runt"])
def listar_detalles_agrupados():
    """
    Obtiene todos los detalles agrupados por cédula de RUNT para mostrar.
    - Llama a función DAL para traer datos agrupados.
    - Retorna JSON con los datos.
    - Captura y reporta errores con código 500.
    """
    try:
        datos = obtener_detalles_agrupados_Runt()
        return {"data": datos}
    except Exception as e:
        import traceback
        traceback.print_exc()
...
def notificar_finalizacion(idEncabezado: int = Body(..., embed=True)):
    """
    Envía un correo notificando la finalización del proceso Runt al usuario que subió la base.
    """
    try:
...
#------------- PAUSAR----------------------------
@router.post("/pausar/{id_encabezado}", tags=["Runt"])
...
@router.post("/reanudar/{id_encabezado}", tags=["Runt"])

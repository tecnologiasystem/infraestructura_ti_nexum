...
const RpaRunt = () => {
  ...
  // ===== data agrupada (si la usas en otro sitio) =====
  ...
  // ===== encabezados: solo 5 por página (server-side) =====
  const cargarAutomatizaciones = async (
    ...
    try {
      ...
      // 2) para esos 5, trae su resumen real
  ...
  // ===== refrescar una fila (usa resumen real y notifica solo si terminó estrictamente) =====
  ...
  // ===== detalles paginados (modal) con filtro y fallback =====
  const traerDetalles = async (idEncabezado, page, cc, pageSize) => {
    ...
    try {
      ...
      const base = (full?.detalles || [])
        ...
        .filter(
          (d) => String(d.placaVehiculo || "").toLowerCase() !== "pausado"
        );

      const filtrados =
        cc && cc.trim()
          ? base.filter((d) =>
              String(d.cedula || d.CC || "").includes(cc.trim())
            )
          : base;
    ...
    <div className="rpa-vigilancia-container">
      <Card className="rpa-main-card">
        ...
        <div className="rpa-header">
          <h1 className="excel-tittle">RPA Runt</h1>
          <p className="excel-description">
            Automatización del proceso de consulta y registro del Registro Único Nacional de Tránsito.
          </p>
        </div>
      ...
      >
        ...
        <Table
          className="rpa-table"
          rowKey={(r, i) => `${r.idDetalle || i}-${r.cedula || r.CC || ""}`}

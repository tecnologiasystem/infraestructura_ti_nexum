...
const RpaSimit = () => {
  // ===== estados generales / agrupado opcional =====
  ...
  // ===== data agrupada (si la usas en otra parte) =====
  ...
  const cargarAutomatizaciones = async (
    ...
    try {
      ...
      // 2) para esos 5 filas, trae el resumen real
  ...
  const traerDetalles = async (idEncabezado, page, cc, pageSize) => {
    ...
    try {
      ...
      const base = (full?.detalles || [])
        .filter((d) => d?.tipo || d?.placa || d?.secretaria)
        .filter((d) => String(d?.placa || "").toLowerCase() !== "pausado");
    ...
    <div className="rpa-vigilancia-container">
      <Card className="rpa-main-card">
        ...
        <div className="rpa-header">
          <h1 className="excel-tittle">RPA Simit</h1>
          <p className="excel-description">
            Automatización del proceso de consulta y registro de información del SIMIT.
          </p>
        </div>
      ...
      >
        ...
        <Table
          ...
          columns={[
            ...
            { title: "Comparendo", dataIndex: "comparendo", key: "comparendo" },
